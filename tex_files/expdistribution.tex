...
\opt{solutionfiles}{\subsection*{Theory and Exercises}
...
\begin{exercise}\clabel{ex:54}
 ...
 Assume that the inter-arrival times $\{X_i\}$ are i.i.d.\ with exponential distribution with mean $1/\lambda$ for some $\lambda>0$.
...
Use the standard integral 
 $\int_0^\infty x^n e^{-\alpha x} \d x = \alpha^{-n-1}n! $. 
