...
\opt{solutionfiles}{\Opensolutionfile{hint}
...
\begin{align*}
 d_{A,k} &= \min\{L_{A,k-1}, 2c_k\}, \quad(\text{as faulty jobs require half of the processing time})\\
 c_{B,k} &= c_k - d_{A,k}/2, \\
...
\begin{equation}
 ...
 L_k^A &= L_{k-1}^A -d_k^A + a_k, \\
 a_k^B &= d_{k}^A, \quad \text{ensures all jobs first pass station A and then station B}\\
 d^B_k &= \min\{L_{k-1}^B, c_k^B\}, \\
 L_k^B &= L_{k-1}^B -d_k^B + a_k^B.
 \end{split}
\end{equation}
...
\begin{extra}[Merging incoming streams]
 ...
 Let $\{a_k^a\}$ be the number of arrivals of stream $a$ and $\{a_k^b\}$ be the number of arrivals of stream $b$ in period $k$.
